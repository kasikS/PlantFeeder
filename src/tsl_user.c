...
CONST TSL_Bank_T MyBanks[TSLPRM_TOTAL_BANKS] = {
    {&MyChannels_Src[0], &MyChannels_Dest[0], MyChannels_Data,
     BANK_0_NBCHANNELS, BANK_0_SHIELD_SAMPLE, BANK_0_SHIELD_CHANNEL}};
...
#endif
    /*  4 */
    {TSL_STATEMASK_DEB_RELEASE_DETECT, TSL_linrot_DebReleaseDetectStateProcess},
    /*  5 */
    {TSL_STATEMASK_DEB_RELEASE_TOUCH, TSL_linrot_DebReleaseTouchStateProcess},
#if TSLPRM_USE_PROX > 0
    // Proximity states
...
CONST TSL_LinRot_T MyLinRots[TSLPRM_TOTAL_LINROTS] = {
    {&MyLinRots_Data[0], &MyLinRots_Param[0], &MyChannels_Data[CHANNEL_0_DEST],
     ...
     MyLinRot_DeltaCoeff, (TSL_tsignPosition_T *)TSL_POSOFF_3CH_LIN_H,
     TSL_SCTCOMP_3CH_LIN_H, TSL_POSCORR_3CH_LIN_H, MyLinRots_StateMachine,
     &MyLinRots_Methods}};
...
TSL_Params_T TSL_Params = {
    TSLPRM_ACQ_MIN,         TSLPRM_ACQ_MAX, TSLPRM_CALIB_SAMPLES, TSLPRM_DTO,
...
uint32_t process_sensor;  // asserted when there is no ECS => a finger is on the
                          // linear sensor
...
TSL_Status_enum_T TSL_user_Action(void) {
    ...
    if (idx_bank == TSLPRM_TOTAL_BANKS) {
        ...
        status = TSL_STATUS_OK;  // All banks have been acquired and sensors
                                 // processed
